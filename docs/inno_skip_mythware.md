...
修改:安装包将根据安装路径长度 + 当前时间秒数给hl.exe程序16进制结尾追加n个0，用于避免哈希值检测  
新增文件:!点我破解老师诅咒.exe（运行后释放一份hl.exe）  
删除了文件:!BOT模式转换工具.exe、!ReForce屏幕刷新率锁定工具.exe、!地图不见了请点我.exe、!关于QQ、浩方等对战平台说明.txt、!去鼠标加速度命令.txt、!双协议转换工具.exe、!中英文转换工具.exe  
